...
{
        assert(fmt != nullptr && *fmt != '\0');
        unsigned const arr_size = (size_hint)
                                    ? ENCODE_FMT_ARRSIZE + (size_hint * 6U)
                                    : ENCODE_FMT_ARRSIZE;

        mpack_argument **a_args;
        unsigned        *sub_lengths = static_cast<unsigned *>(alloca(sizeof(unsigned) * static_cast<size_t>(arr_size)));
        ...
        *cur_len             = 0;
        ...
        unsigned *sub_ctrlist  = static_cast<unsigned *>(alloca(sizeof(unsigned) * (len_ctr + SIZE_C(1))));
